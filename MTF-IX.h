...
typedef size_t (*IOFunc)(void *context, void *buffer, size_t length);

bool MICompress(void *input_ctx, IOFunc read, void *output_ctx, IOFunc write);
bool MIDecompress(void *input_ctx, IOFunc read, void *output_ctx, IOFunc write);
...
void MIWriteStreamInit(MIWriteStream *self, void *write_context, IOFunc write) {
	memset(self, 0, sizeof *self);
	self->write = write;
	self->context = write_context;
}

bool MIFlush(MIWriteStream *self) {
...
bool MIWriteHd(MIWriteStream *self, char hexdigit) {
	/**
...
bool MIWriteCh(MIWriteStream *self, char ch) {
	/**
...
void MIReadStreamInit(MIReadStream *self, void *context, IOFunc read) {
	memset(self, 0, sizeof *self);
	self->read = read;
	self->context = context;
}

bool MIEnsureDataIsAvailable(MIReadStream *self) {
...
bool MIReadHd(MIReadStream *self, char *ch) {
...
bool MIReadCh(MIReadStream *self, char *ch) {
	if (!MIEnsureDataIsAvailable(self)) {
		return false;
	}
...
void MIMtfInit(MIMtf *mtf) {
	for (size_t i = 0; i < 256; i++) {
		mtf->stack[i] = i;
	}
}

uint8_t MIMtfIndexForCh(MIMtf *mtf, char ch) {
...
char MIMtfChForIndex(MIMtf *mtf, uint8_t i) {
...
bool MICompressUsingInternalStreams(MIReadStream *input, MIWriteStream *output) {
	MIMtf mtf;
...
bool MIDecompressUsingInternalStreams(MIReadStream *input, MIWriteStream *output) {
	MIMtf mtf;
